...
function upgrade30_dbchanges()
{
	...
	echo "<p>انجام کوئری‌های مورد نیاز برای بروزرسانی...</p>";
	flush();
	...
	global $footer_extra;
	$footer_extra = "<script type=\"text/javascript\">$(document).ready(function() { var button = $('.submit_button'); if(button) { button.val('انتقال خودکار...'); button.prop('disabled', true); button.css('color', '#aaa'); button.css('border-color', '#aaa'); document.forms[0].submit(); } });</script>";
...
function upgrade30_dbchanges2()
{
	...
	echo "<p>درحال اجرای کوئری‌های مورد نیاز برای بروزرسانی...</p>";
	flush();
...
function upgrade30_dbchanges3()
{
	...
	echo "<p>درحال انجام کوئری‌های مورد نیاز برای بروزرسانی...</p>";
	flush();
...
function upgrade30_dbchanges6()
{
	...
	echo "<p>درحال بروزرسانی دسترسی‌های گروه‌های کاربری...</p>";
	$groups = array();
...
<option value=\"+14\" ".($setting[\'value\'] == 14?"selected=\"selected\"":"").">GMT +14:00 Hours (".my_date($mybb->settings[\'timeformat\'], TIME_NOW, 14).")</option>
	...
	echo "<p> {$added_tasks} وظیفه‌ی جدید اضافه شد.</p>";
...
function upgrade30_threadcount()
{
	...
	$output->print_header("بازشماری تعداد موضوعات کاربران");
	...
	echo "<p>درحال شمارش موضوع برای کاربر #{$lower} به #{$upper} ({$cnt['usercount']} در مجموع)</p>";
	flush();
	...
	else
	{
		...
		$contents = "<p>انجام شد</p><p>تعداد موضوع همه ی کاربران شمارش شد. برای ادامه برروی بعدی کلیک کنید.</p>";
	}
...
function upgrade30_dbchanges_optimize1()
{
	...
	$output->print_header("بهینه‌سازی پایگاه داده");

	echo "<p>درحال اجرای کوئری‌های مورد نیاز برای بهینه‌سازی پایگاه داده...</p>";
	flush();
...
function upgrade30_dbchanges_optimize2()
{
	...
	echo "<p>درحال اجرای کوئری‌های بهینه‌سازی مورد نیاز...</p>";
	echo "<p>درحال اضافه‌کردن indexeها به جدول‌ها...</p>";
	flush();
	...
	echo "<p>حذف indexeهای قدیمی از جدول‌ها...</p>";
...
function upgrade30_dbchanges_optimize3()
{
	...
	{
		echo "<p>{$table}: درحال تبدیل نوع ستون به tinyint</p>";
		$change_column = array();
...
function upgrade30_dbchanges_optimize4()
{
	...
	{
		echo "<p>{$table}: درحال تبدیل نوع ستون به int</p>";
		$change_column = array();
		foreach($columns as $column)
		{
			if($db->type == "pgsql")
...
function upgrade30_dbchanges_smilies()
{
	...
	$output->print_header("بروزرسانی شکلک‌ها");
...
function upgrade30_dbchanges_ip()
{
	...
	$output->print_header("تبدیل‌سازی IP");
	...
	{
		case 8:
			echo "<p>درحال اضافه‌کردن شاخصه‌های پایگاه‌داده (3/3)...</p>";
			flush();
			...
			break;
		case 7:
			echo "<p>درحال اضافه کردن شاخصه‌های پایگاه داده (2/3)...</p>";
			flush();
			...
			break;
		case 6:
			echo "<p>درحال اضافه کردن شاخصه‌های پایگاه داده (1/3)...</p>";
			flush();
		...
		case 5:
			...
			{
				case 7:
					echo "<p>درحال تبدیل IPهای کاربران...</p>";
					flush();
			...
			echo "<p>درحال تبدیل ip {$lower} به {$upper} ({$cnt['ipcount']} Total)</p>";
			flush();
		...
		case 4:
			...
			{
				case 10:
					echo "<p>درحال بروزرسانی جدول کاربر (4/4)...</p>";
					flush();
					...
					break;
				case 9:
					echo "<p>درحال بروزرسانی جدول کاربر (3/4)...</p>";
					flush();
					...
					break;
				case 8:
					echo "<p>درحال بروزرسانی جدول رتبه‌های موضوع...</p>";
					flush();
					...
					break;
				case 7:
					echo "<p>درحال بروزرسانی جدول جلسه‌ها...</p>";
					flush();
					...
					break;
				case 6:
					echo "<p>درحال بروزرسانی جدول اعمال جستجو‌ها...</p>";
					flush();
					...
					break;
				case 5:
					echo "<p>درحال بروزرسانی جدول ارسال‌ها (2/2)...</p>";
					flush();
					...
					break;
				case 4:
					echo "<p>درحال بروزرسانی جدول moderatorlog ...</p>";
					flush();
					...
					break;
				case 3:
					echo "<p>درحال بروزرسانی جدول maillog...</p>";
					flush();
					...
					break;
				case 2:
					echo "<p>درحال بروزرسانی جدول adminsession...</p>";
					flush();
					...
					break;
				default:
					echo "<p>درحال بروزرسانی جدول adminlog...</p>";
					flush();
			...
			break;
		case 3:
			echo "<p>درحال بروزرسانی جدول کاربر (2/4)...</p>";
			flush();
			...
			break;
		case 2:
			echo "<p>درحال بروزرسانی جدول کاربر (1/4)...</p>";
			flush();
			...
			break;
		default:
			echo "<p>درحال بروزرسانی جدول ارسال‌ها (1/2)...</p>";
			flush();
...
function upgrade30_updatetheme()
{
	...
	$output->print_header("بروزرسانی پوسته‌ها");
...
function upgrade30_acppin()
{
	...
	$output->print_header("افزودن کد PIN");

	echo "<p>ما یک ویژگی امنیتی جدید در 1.8 اضافه کرده‌ایم: امکان قرار دادن کد امنیتی PIN که نیاز هست در ACP وارد شود.<br />\n";
	echo "اگر شما نمی‌خواهید PINای را تنظیم کنید این مرحله را نادیده بگیرید (فیلد زیر را خالی نگه دارید). شما می‌توانید PIN را بعدا تنظیم کنید (مستندات را در پایین ببینید).</p>\n";
	echo '<div class="border_wrapper">
			<div class="title">پیکربندی PIN پنل مدیریت</div>
			<table class="general" cellspacing="0">
				<tbody>
				<tr>
					<th colspan="2" class="first last">PIN امنیتی پنل مدیریت</th>
				</tr>
				<tr class="first">
...
function upgrade30_acppin_submit()
{
	...
	$output->print_header("نوشتن فایل پیکربندی");

	$content = "<p>حالا ما کد PIN (اگر شما یکی را وارد کرده باشید) را در فایل config.php می‌نویسیم... ";
