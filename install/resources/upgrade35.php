...
function upgrade35_dbchanges()
{
	...
	echo "<p>انجام کارهای لازم برای بروزرسانی...</p>";
	flush();
	...
	global $footer_extra;
	$footer_extra = "<script type=\"text/javascript\">$(document).ready(function() { var button = $('.submit_button'); if(button) { button.val('هدایت خودکار...'); button.prop('disabled', true); button.css('color', '#aaa'); button.css('border-color', '#aaa'); document.forms[0].submit(); } });</script>";

	$output->print_contents("<p>بر روی بعدی کلیک کنید تا فرایند بروزرسانی را ادامه دهید.</p>");
...
function upgrade35_dbchanges2()
{
	...
	$output->print_header("بروزرسانی پایگاه داده");
	echo "<p>انجام کارهای لازم برای بهینه‌سازی پایگاه داده...</p>";
	flush();
...
function upgrade35_dbchanges4()
{
	...
	$output->print_header("افزودن فایل‌های index");
	echo "<p>افزودن فایل index به پوشه‌های پیوست‌ها...</p>";
	flush();
