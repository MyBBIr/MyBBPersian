...
function fetch_forum_announcements(&$table, $pid=0, $depth=1)
{
	...
	foreach($forums_by_parent[$pid] as $children)
	{
		foreach($children as $forum)
		{
			...
			$table->construct_cell("<div style=\"padding-right: ".(40*($depth-1))."px;\">{$forum['name']}</div>");
			...
			if(isset($announcements[$forum['fid']]))
			{
				foreach($announcements[$forum['fid']] as $aid => $announcement)
				{
					...
					$table->construct_cell("<div style=\"padding-right: ".(40*$depth)."px;\">{$icon}<a href=\"index.php?module=forum-announcements&amp;action=edit&amp;aid={$aid}\">".htmlspecialchars_uni($announcement['subject'])."</a></div>");
