...
if($mybb->input['action'] == "add")
{
	...
	$form_container = new FormContainer("<div class=\"float_left\" style=\"font-weight: normal;\"><a href=\"#\" onclick=\"$('#additional_options_link').toggle(); $('#additional_options').fadeToggle('fast'); return false;\">{$lang->hide_additional_options}</a></div>".$lang->additional_forum_options);
	...
	foreach($forums_by_parent[$pid] as $children)
	{
		foreach($children as $forum)
		{
			...
			if($forum['type'] == "c" && ($depth == 1 || $depth == 2))
			{
				...
				$form_container->output_cell("<div style=\"padding-right: ".(40*($depth-1))."px;\"><a href=\"index.php?module=forum-management&amp;fid={$forum['fid']}\"><strong>{$forum['name']}</strong></a>{$sub_forums}</div>");
			...
			elseif($forum['type'] == "f" && ($depth == 1 || $depth == 2))
			{
				...
				$form_container->output_cell("<div style=\"padding-right: ".(40*($depth-1))."px;\"><a href=\"index.php?module=forum-management&amp;fid={$forum['fid']}\">{$forum['name']}</a>{$forum['description']}{$sub_forums}</div>");
