	...
	if($mybb->input['step'] == 3)
	{
		...
		if(is_array($bad_attachments))
		{
			foreach($bad_attachments as $file)
			{
				...
				$table->construct_cell("<span class=\"float_left\">{$filesize}</span>{$file}");
		...
		if(count($aids) > 0)
		{
			...
			while($attachment = $db->fetch_array($query))
			{
				...
				$table->construct_cell(get_attachment_icon(get_extension($attachment['filename'])), array('width' => 1));
				$table->construct_cell("<span class=\"float_left\">".get_friendly_size($attachment['filesize'])."</span>{$attachment['filename']}", array('class' => $cell_class));
