...
if($mybb->input['action'] == "version_check")
{
	...
	if($feed_parser->error == '')
	{
		foreach($feed_parser->items as $item)
		{
			...
			$table->construct_cell("<span style=\"font-size: 16px;\"><strong>".htmlspecialchars_uni($item['title'])."</strong></span><br /><br />{$content}<strong><span style=\"float: left;\">{$stamp}</span><br /><br /><a href=\"{$link}\" target=\"_blank\">&raquo; {$lang->read_more}</a></strong>");
			$table->construct_row();
		}
	}
	else
