	...
	else
	{
		...
		switch($mybb->input['inline_action'])
		{
			...
			case 'multiban':
				...
				$form_container = new FormContainer('<div class="float_left"><a href="index.php?module=user-users&amp;action=inline_edit&amp;inline_action=multilift&amp;my_post_key='.$mybb->post_code.'">'.$lang->lift_bans.'</a></div>'.$lang->mass_ban);
...
if(!$mybb->input['action'])
{
	...
	if($errors)
	{
		if($inline != true)
		{
			echo "<div style=\"display: inline; float: left;\">{$admin_view['popup']}</div><br />\n";
		}
		$page->output_inline_error($errors);
	}
...
function build_users_view($view)
{
	...
	$switch_view = "<div class=\"float_left\">";
	$switch_url = $view['url'];
	...
	if($num_results > $view['perpage'])
	{
		...
		$search_class = "float_left";
		$search_style = "";
	}
	else
	{
		$search_class = '';
		$search_style = "text-align: left;";
	}
...
<div class="float_left"><span class="smalltext"><strong>'.$lang->inline_edit.'</strong></span>
...
function build_user_view_card($user, $view, &$i)
{
	...
	if($controls)
	{
		$card .= "<div class=\"float_left\" style=\"padding: 4px;\">{$controls}</div>\n";
	}
	$card .= "</fieldset>";
	return $card;
