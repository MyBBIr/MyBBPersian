	...
	while($group = $db->fetch_array($query))
	{
		...
		if(is_array($settings_cache[$group['gid']]))
		{
			foreach($settings_cache[$group['gid']] as $setting)
			{
				...
				$table->construct_cell($setting_title, array('style' => 'padding-right: 40px;'));
...
EOF;
	...
	// Search form
	echo "<div style=\"text-align: left; margin-bottom: 3px;\">";
	...
	$table->output("<span style=\"float: left;\"><small><a href=\"index.php?module=config-settings&amp;action=change\">{$lang->show_all_settings}</a></small></span>{$lang->board_settings}");
